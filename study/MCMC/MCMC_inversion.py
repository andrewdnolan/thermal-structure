...
def plot_BAMR(z: np.ndarray, ppc: tuple, obs: tuple, out_fp: str, axis_labels=None):
    ...
    if axis_labels == None:
        axis_labels = ('Refreezing (m/yr)','Accumulation (m/yr)','Melt (m/yr)', 'Refreezing (m/yr)')
...
def simultaneous_fit_LA(z_obs, R_obs, A_obs, M_obs, draws=4000, tune=2000, cores=1):
...
def new_likelihood(z_obs, R_obs, A_obs, M_obs, draws=4000, tune=2000, cores=1):
    ...
    with model:
        # Individual likelihood functions for each component
        est = pm.Normal("est", mu=np.array([R, A, M]), sigma=sigma, observed=theano.shared([R_obs, A_obs, M_obs]))
...
def run_LA(fit_type, draws=4000, tune=2000, cores=1):
    ...
    if simul:
        ...
        model, trace, pp_R, pp_A, pp_M, pp_B= simultaneous_fit_LA(z_obs, R_obs, A_obs,
                                                             M_obs, draws, tune,
                                                             cores)
    elif new:
        model, trace, pp_R, pp_A, pp_M, pp_B = new_likelihood(z_obs, R_obs, A_obs,
                                                             M_obs, draws, tune,
                                                             cores)
    # plot the traces
    ...
    if simul:
        ...
        plot_BAMR(z_obs, (pp_R, pp_A, pp_M, pp_B), (R_obs, A_obs, M_obs, B_obs),
        out_fp=out_fp)
    elif new:
...
if __name__ == '__main__':
    ...
    with model:
        # Individual likelihood functions for each component
        est = pm.Normal("est", mu=tt.log(mu.var()), sigma=sigma, observed=theano.shared([R_obs, A_obs, M_obs]))
